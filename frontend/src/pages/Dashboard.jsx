...
const PILL_COLORS = {
    ...
    primary: { bg: 'bg-primary', glow: 'shadow-primary/25', light: 'bg-primary/10', text: 'text-primary', border: 'border-primary/20' },
...
const KPICard = ({ label, value, icon: Icon, color = 'primary', trend, sub }) => {
    ...
    return (
        <div className="group relative bg-white flex flex-col justify-between rounded-[22px] p-5 border border-slate-100/80 hover:border-slate-200 transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] cursor-default overflow-hidden">
            ...
            <div className="relative z-10">
                ...
                <p className="text-[26px] font-black text-[#111111] tracking-tight leading-none">{value}</p>
    ...
    return (
        <div className="space-y-7 animate-fadeIn pb-12">
            ...
            <div className="relative bg-white rounded-[26px] p-7 border border-slate-100/80 overflow-hidden">
                <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-primary via-sky-500 to-emerald-400" />
                <div className="absolute -right-20 -top-20 w-60 h-60 bg-gradient-to-br from-primary/10 to-sky-500/10 rounded-full blur-3xl animate-pulse" />
                <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div>
                        <h1 className="text-[28px] font-black text-[#111111] tracking-tight flex items-center gap-2.5">
                            Command Center
                        ...
                        <p className="text-[13px] font-medium text-slate-400 mt-1">
                            Welcome back, <span className="font-bold text-primary">{user?.name}</span> — Master Overview
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <div className="relative z-10 w-full h-[320px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={g.monthlyData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                                    ...
                                    <Area type="monotone" dataKey="revenue" stroke="#0047FF" strokeWidth={2.5} fillOpacity={1} fill="url(#revGrad2)" dot={false} activeDot={{ r: 5, fill: '#0047FF', strokeWidth: 2, stroke: '#fff' }} />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
